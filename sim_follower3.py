...
class DroneSubscriber(Node):
    ...
    def status_callback(self,msg):
        ...
        if self.status == 'ready':
            ...
            if self.counter == 1:
                ...
                formation(follower1,self.leader_lat,self.leader_lon,self.leader_hdg,225,8.1,target_alt)
                time.sleep(1)
                lat,lon,alt = get_global_position(follower1)
                new_alt = target_alt + 15
    ...
    def localpos_callback(self,msg):
        ...
        self.pos_y = msg.y
        self.pos_z = msg.z
        self.get_logger().info(f"local position: x={self.pos_x} m, vy={self.pos_y} m, vz={self.pos_z} m")
        if abs(self.pos_z)<5:
...
follower1 = connect('tcp:127.0.0.1:5803')
print("connected")
time.sleep(1)
target_alt = 5

...
class DroneSubscriber(Node):
    ...
    def velocity_callback(self, msg):
        ...
        self.get_logger().info(f"leader velocity: vx={self.leader_vx} m, y={self.leader_vy} m, z={self.leader_vz} m")
    ...
    def status_callback(self,msg):
        ...
        if self.status == 'ready':
            ...
            if self.counter == 1:
                ...
                formation(follower1,self.leader_lat,self.leader_lon,self.leader_hdg,180,5,target_alt)
                time.sleep(1)
                lat,lon,alt = get_global_position(follower1)
                new_alt = target_alt + 5
    ...
    def localpos_callback(self,msg):
        ...
        self.pos_y = msg.y
        self.pos_z = msg.z
        self.get_logger().info(f"local position: x={self.pos_x} m/s, vy={self.pos_y} m/s, vz={self.pos_z} m/s")
        if abs(self.pos_z)<5:
            VehicleMode(follower1,"LAND")
            time.sleep(1)

   
follower1 = connect('tcp:127.0.0.1:5793')
print("connected")
time.sleep(1)
target_alt = 15
